...
reviewMiddleware.Review = {
  fetchProductReview: async ({ body }) => {
    ...
    if (
      ...
      //review data
      reviewData.excellent = Math.round(
        (reviewData.excellent * 100) / reviewData.noOfRatings
      );
      reviewData.best = Math.round(
        (reviewData.best * 100) / reviewData.noOfRatings
      );
      reviewData.good = Math.round(
        (reviewData.good * 100) / reviewData.noOfRatings
      );
      reviewData.poor = Math.round(
        (reviewData.poor * 100) / reviewData.noOfRatings
      );
      reviewData.verypoor = Math.round(
        (reviewData.verypoor * 100) / reviewData.noOfRatings
      );
  ...
  fetchReviews: async ({ token }) => {
    ...
    if (fetched.length != 0) {
      for (let index = 0; index < fetched.length; index++) {
        ...
        fetched[index].createdAt = moment(
          fetched[index].reviews.createdAt
        ).fromNow();
  ...
  createReview: async ({ body, token }) => {
    ...
    if (
      ...
      if (
        ...
        const updateReview = await userDbController.Review.updateOrderReview(
          body
        );
